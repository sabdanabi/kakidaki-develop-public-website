...
filesToProcess.forEach(relativePath => {
  ...
  // 2. Remove tracking utilities that might conflict
  content = content.replace(/\btracking-(wide|wider|widest|tight)\b/g, '');
  ...
  console.log(`Successfully processed typography: ${relativePath}`);
});
